...
def test_same_version_software_change_is_not_silently_skipped(
    ...
    monkeypatch.setattr(remote_control, "_current_version", lambda: "0.13.1")
    calls: list[list[str]] = []
...
def test_repository_software_update_uses_the_declared_ref(
    ...
    assert calls
    assert "git+https://github.com/yunlux/Anet@v0.13.1" in calls[0]
...
def test_failed_software_update_restores_the_active_package(
    ...
    metadata = runtime / "site-packages" / "anet_fabric-0.13.1.dist-info"
...
def test_initial_same_version_software_is_recorded_without_reinstall(
    ...
    software = {"version": "0.13.1", "wheel_url": "https://example.invalid/initial.whl"}
    monkeypatch.setattr(remote_control, "_current_version", lambda: "0.13.1")
    monkeypatch.setattr(
        remote_control,
        "_download",
