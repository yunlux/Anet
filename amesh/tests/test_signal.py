...
from anet.social import score_social_actor
...
def test_build_and_validate_signal() -> None:
    ...
    assert signal["protocol"] == "anet.social.loopback"
...
def test_validate_rejects_malformed() -> None:
    ...
    with pytest.raises(ValueError):
        validate_signal({**signal, "protocol": "anet.other"})
    with pytest.raises(ValueError):
...
def test_directory_sink_round_trip(tmp_path) -> None:
    ...
    assert payload["protocol"] == "anet.social.loopback"
