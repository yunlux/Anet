...
class AgentStore:
    ...
    def register(self, agent_id: str, name: str, *, scopes: tuple[str, ...] | list[str] = ()) -> dict[str, Any]:
        ...
        self._conn.execute(
            ...
            (agent_id, name, hashlib.sha256(token.encode()).hexdigest(), json.dumps(normalized), current, current),
        )
    ...
    def get(self, agent_id: str) -> AgentRecord | None:
        row = self._conn.execute("SELECT * FROM amesh_agents WHERE agent_id=?", (_agent_id(agent_id),)).fetchone()
        if row is None:
            return None
        return AgentRecord(str(row["agent_id"]), str(row["name"]), tuple(json.loads(row["scopes_json"])), bool(row["enabled"]), int(row["created_ms"]), int(row["last_seen_ms"]))

    def list(self) -> list[AgentRecord]:
        rows = self._conn.execute("SELECT agent_id FROM amesh_agents ORDER BY agent_id").fetchall()
        return [self.get(str(row["agent_id"])) for row in rows]  # type: ignore[list-item]

    def revoke(self, agent_id: str) -> bool:
        changed = self._conn.execute("UPDATE amesh_agents SET enabled=0 WHERE agent_id=?", (_agent_id(agent_id),)).rowcount
        return changed == 1

    def authenticate(self, token: str) -> AgentRecord:
        digest = hashlib.sha256(str(token).encode()).hexdigest()
        row = self._conn.execute("SELECT agent_id FROM amesh_agents WHERE token_hash=? AND enabled=1", (digest,)).fetchone()
        if row is None:
            raise PermissionError("invalid or disabled Amesh agent token")
        agent_id = str(row["agent_id"])
        self._conn.execute("UPDATE amesh_agents SET last_seen_ms=? WHERE agent_id=?", (int(time.time() * 1000), agent_id))
        record = self.get(agent_id)
        if record is None:  # pragma: no cover
            raise PermissionError("Amesh agent disappeared")
        return record

    def grant(self, agent_id: str, adapter: str, action: str, effect: str, *, reason: str = "") -> dict[str, Any]:
        agent_id = _agent_id(agent_id)
        ...
        self._conn.execute("INSERT INTO amesh_agent_grants VALUES (?, ?, ?, ?, ?, ?, ?)", (grant_id, agent_id, adapter, action, effect, str(reason)[:256], int(time.time() * 1000)))
        return {"grant_id": grant_id, "agent_id": agent_id, "adapter": adapter, "action": action, "effect": effect, "reason": str(reason)[:256]}
    ...
    def authorize(self, agent_id: str, adapter: str, action: str) -> bool:
        ...
        if record is None or not record.enabled or action not in record.scopes:
            return False
        row = self._conn.execute("SELECT effect FROM amesh_agent_grants WHERE agent_id=? AND adapter=? AND action=? ORDER BY created_ms DESC, grant_id DESC LIMIT 1", (agent_id, str(adapter).lower(), str(action).lower())).fetchone()
