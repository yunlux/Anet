...
from ..relations import ActorObservation, ActorProof, InteractionEvidence, RelationshipHub
...
class DiscordAdapter(PlatformAdapter):
    ...
    def descriptor(self) -> dict[str, Any]:
        if not self.configured:
            return {"name": self.name, "configured": False, "enabled": False, "summary": "no discord.json configured"}
        config = self._config()
        return {
            "name": self.name,
            "configured": True,
    ...
    def status(self) -> dict[str, Any]:
        ...
        runtime: dict[str, Any] = {"runtime_state": "never_run", "last_error_category": "", "consecutive_failures": 0, "next_retry_ms": 0}
        store = self._open_store()
        if store is not None:
            counts.update(store.status())
            runtime.update(store.runtime_status())
        return {"name": self.name, "configured": True, "enabled": config.enabled, **runtime, **counts, "permission_rules": len(self.permission_rules())}
    ...
    def actor(self, actor_key: str) -> dict[str, Any]:
        ...
        gated, reasons = self.permitted_actions(actor_key, list(evaluation["allowed_actions"]))
        return {**stats, "evaluation": evaluation, "permission": {"effective_allowed_actions": gated, "reasons": reasons, "rules": self.permission_rules(actor_key)}}

    def set_labels(self, actor_key: str, *, add, remove, source: str = "operator") -> dict[str, Any]:
        actor_key = validate_actor_key(actor_key)
        self._config()
        store = self._open_store()
        if store is None:
    ...
    def project(self, *, limit: int = 1000) -> dict[str, Any]:
        ...
        return {"events_examined": len(projections), "interactions_recorded": sum(1 for item in projections if item["recorded"]), "actors": sorted({item["actor_id"] for item in projections}), "note": "Discord evidence created no agent grant or authorization"}

    def project_event(self, event: Mapping[str, Any]) -> dict[str, Any]:
        hub = self._relationship_hub()
        actor_id = platform_actor_id("discord", namespace_actor_id=hub.identity.identity_id, platform_actor_key=str(event["actor_key"]))
        ...
        subject = hub.book.observe_typed_actor(
            ActorObservation(
                ...
                proof=ActorProof(proof_type="discord.rest.v1", scope="platform-observed", issuer_actor_id=hub.identity.identity_id, evidence_ref=evidence_ref, observed_ms=int(event["created_ms"])),
            ),
            subject_confidence=50,
            now=int(event["created_ms"]),
        )
        recorded = hub.book.record_interaction(InteractionEvidence.create(actor_id=actor_id, subject_ref=subject.subject_ref, direction="incoming", facets={"message"}, context="social.discord", outcome="received", evidence_ref=evidence_ref, occurred_ms=int(event["created_ms"])))
        relationship = hub.book.relationship(subject.subject_ref)
        if relationship is None:
            raise RuntimeError("Discord relationship disappeared")
        if relationship.circle == "public" and {"interaction:mention", "interaction:reply"}.intersection(event.get("event_labels", ())):
            relationship = hub.book.set_circle(subject.subject_ref, "known", confidence=25, evidence_ref=evidence_ref, labels=("interaction:directed", "platform:discord"), now=int(event["created_ms"]))
        return {"actor_id": actor_id, "subject_ref": subject.subject_ref, "recorded": recorded, "circle": relationship.circle}

    def relation(self, actor_key: str) -> dict[str, Any]:
        actor_key = validate_actor_key(actor_key)
        hub = self._relationship_hub()
        actor_id = platform_actor_id("discord", namespace_actor_id=hub.identity.identity_id, platform_actor_key=actor_key)
        subject = hub.book.primary_subject(actor_id)
        return {"observed": False, "actor_id": actor_id, "platform": self.name} if subject is None else {"observed": True, "actor_id": actor_id, "platform": self.name, "subject_ref": subject.subject_ref, "relationship": hub.book.relationship(subject.subject_ref).to_dict()}
    ...
    def _gated_queue_signal(self, queue_signal: Callable[[str, str, dict[str, Any]], str] | None):
        def _gate(destination_id: str, kind: str, body: dict[str, Any]) -> str:
            ...
            if self.permission_denies(actor_key, "surface"):
                self.record_permission_decision(actor_key, "surface", "deny", event_key=str(body.get("source_event_id", "")))
                return hashlib.sha256(f"amesh:blocked:{body.get('source_event_id', '')}".encode("utf-8")).hexdigest()[:32]
    ...
    def poll_once(self, queue_signal=None, project_event=None) -> dict[str, Any]:
        return self._load_bridge().poll_once(self._gated_queue_signal(queue_signal), project_event or self.project_event)

    async def run(self, stop: Any, queue_signal, project_event=None) -> None:
        await self._load_bridge().run(stop, self._gated_queue_signal(queue_signal), project_event or self.project_event)
    ...
    def reply(self, event_key: str, content: str) -> dict[str, Any]:
        ...
        if self.permission_denies(event["actor_key"], "reply"):
            self.record_permission_decision(event["actor_key"], "reply", "deny", event_key=event_key)
            raise PermissionError(f"reply blocked by an Amesh permission rule for {event['actor_key'][:8]}…")
