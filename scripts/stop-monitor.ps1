...
if (-not (Test-Path -LiteralPath $pidFile) -and -not (Test-Path -LiteralPath $launcherPidFile)) {
    Write-Output "Anet monitor is not running (no PID file)."
    exit 0
}
...
$depthByPid = @{}
function Add-AnetMonitorTree([int]$ProcessId, [int]$Depth) {
    ...
    foreach ($child in $allProcesses | Where-Object { $_.ParentProcessId -eq $ProcessId }) {
        Add-AnetMonitorTree -ProcessId ([int]$child.ProcessId) -Depth ($Depth + 1)
    }
}
foreach ($seedPid in $seedPids | Select-Object -Unique) {
    ...
    if ($seed -and ($seed.CommandLine -notlike "*-m anet*" -or $seed.CommandLine -notlike "* monitor *")) {
        throw "Refusing to stop PID $seedPid because it is not an Anet monitor."
    }
    if ($seed) {
        Add-AnetMonitorTree -ProcessId $seedPid -Depth 0
    }
}
...
Write-Output "Anet monitor stopped: PID=$($seedPids -join ',')"
